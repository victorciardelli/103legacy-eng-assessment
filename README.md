...
| POST | `/api/cards/:cardId/compile` | Compile all videos into final card video |
...
### 7. Captions are silently lost
Contributors can type a caption for their video (e.g., "Happy birthday from the whole family!") which should appear as a subtitle overlay in the final compiled video. The caption field exists in the upload form and the data is sent to the server, but **the server never stores it in the database**. The `INSERT` statement in the upload endpoint simply doesn't include the caption column. When the compile endpoint (`POST /api/cards/:cardId/compile`) runs, it tries to read captions from each upload record and finds nothing — every caption is empty. The organizer dashboard also shows a Caption column that is always "—" with no indication that data was lost. This is one of the most critical bugs because captions are essential to the final product — without them, the compiled video is just a series of anonymous clips with no context about who is speaking or what they want to say.
